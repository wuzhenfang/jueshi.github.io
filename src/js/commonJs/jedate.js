	...
	jedfn.onlyYMStr = function(y, m) {
		...
		$.each(jet.montharr, function(i, val) {
			...
			if (thisDate < minTime || thisDate > maxTime) {
				onlyYM += "<li class='disabled' ym='" + y + "-" + jet.digit(val) + "'>" + jet.digit(val) + "月</li>";
			} else {
				onlyYM += "<li " + (m == val ? 'class="action"' :"") + ' ym="' + y + "-" + jet.digit(val) + '">' + jet.digit(val) + "月</li>";
			}
		});
		return onlyYM;
	};
	...
	jedfn.setStrhms = function(opts) {
		...
		if(parmathm){
			var hmsliWidth = hmsliCls.css('width').replace(/\px|em|rem/g,''), hmsiW = boxCell.find(".jedatehms i").css('width').replace(/\px|em|rem/g,''),
				hmschoseW = proptextCls.css('width').replace(/\px|em|rem/g,''), hmslival = Math.round(parseInt(hmsliWidth) + parseInt(hmsliWidth)/2 - 6  + parseInt(hmsiW)/2 );
		...
		$.each([ ymPre, ymNext ], function(i, cls) {
			cls.on("click", function(ev) {
				...
				if(jet.checkFormat(jet.format) == "YYYY"){
					...
					boxCell.find(".jedateyy .jedateyearmonth").attr("data-onym",ymVal).html(ymVal+"年");
				}
